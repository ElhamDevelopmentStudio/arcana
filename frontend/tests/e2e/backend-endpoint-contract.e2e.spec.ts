...
test.describe('backend real endpoint contract (frontend-integrated)', () => {
  ...
  test('projects/:project_id/run-monitor route loads run detail through backend endpoint', async ({
    page,
    request,
  }) => {
    const title = uniqueTitle('e2e-project-run-monitor-route-contract');
    ...
    expect(runResponse.status()).toBe(200);
    const runPayload = (await runResponse.json()) as { run_id: number; status: string };
        ...
        window.localStorage.setItem(
          ...
          JSON.stringify({
            state: {
              ...
              projectTitle: 'Run monitor contract project',
              selectedMode: 'author',
              chapterCount: 1,
              runId: seededRunId,
            },
    ...
    const runDetailGetRequestPromise = page.waitForRequest(
      (networkRequest) =>
        networkRequest.method() === 'GET' &&
        networkRequest.url().endsWith(`/api/projects/${projectId}/runs/${runPayload.run_id}`),
    );
    ...
    await runDetailGetRequestPromise;
    const runDetailGetResponse = await runDetailGetResponsePromise;
    expect(runDetailGetResponse.status()).toBe(200);

    await expect(page.getByRole('heading', { name: 'Run Monitor' })).toBeVisible();
    await expect(page.getByText(`Run: ${runPayload.run_id}`)).toBeVisible();
    await expect(page.getByText(new RegExp(`Status:\\s*${runPayload.status}`, 'i'))).toBeVisible();
  });

  test('projects/:project_id/export route loads JSON export payload through backend endpoint', async ({
    page,
    request,
  }) => {
    const title = uniqueTitle('e2e-project-export-route-contract');
    const project = await createProject(request, title);
    const projectId = project.id;
      ...
      ({ seededProjectId, seededRunId }) => {
        window.localStorage.setItem(
          ...
          JSON.stringify({
            state: {
              projectId: seededProjectId,
              projectTitle: 'Export route contract project',
              selectedMode: 'author',
              chapterCount: 1,
              runId: seededRunId,
            },
    ...
    const exportGetResponsePromise = page.waitForResponse(
      (networkResponse) =>
        networkResponse.request().method() === 'GET' &&
        networkResponse.url().endsWith(`/api/projects/${projectId}/exports/${runPayload.run_id}.json`),
    );

    await page.goto(`/projects/${projectId}/export`);
    await expect(page.getByRole('heading', { name: 'Exports' })).toBeVisible();

    const exportGetResponse = await exportGetResponsePromise;
    expect(exportGetResponse.status()).toBe(200);
    const exportPayload = (await exportGetResponse.json()) as { project_id: number; run_id: number };
    expect(exportPayload.project_id).toBe(projectId);
  ...
  test('projects/:project_id/export route gates CSV action when run export formats exclude csv', async ({
    page,
    request,
  }) => {
    const title = uniqueTitle('e2e-project-export-route-csv-gating');
    const project = await createProject(request, title);
    const projectId = project.id;
      ...
      ({ seededProjectId, seededRunId }) => {
        window.localStorage.setItem(
          ...
          JSON.stringify({
            state: {
              projectId: seededProjectId,
              projectTitle: 'Export route csv gating project',
              selectedMode: 'author',
              chapterCount: 1,
              runId: seededRunId,
            },
  ...
  test('projects/:project_id/run-monitor route sends recover request through backend endpoint', async ({
    page,
    request,
  }) => {
    const title = uniqueTitle('e2e-project-run-monitor-recover-route-contract');
    const project = await createProject(request, title);
    const projectId = project.id;
      ...
      ({ seededProjectId, seededRunId }) => {
        window.localStorage.setItem(
          ...
          JSON.stringify({
            state: {
              projectId: seededProjectId,
              projectTitle: 'Run monitor recover project',
              selectedMode: 'author',
              chapterCount: 1,
              runId: seededRunId,
            },
    ...
    const recoverPostRequestPromise = page.waitForRequest(
      (networkRequest) =>
        networkRequest.method() === 'POST'
        && networkRequest.url().endsWith(`/api/projects/${projectId}/runs/${runPayload.run_id}/recover`),
    );
    const recoverPostResponsePromise = page.waitForResponse(
      (networkResponse) =>
        networkResponse.request().method() === 'POST'
        && networkResponse.url().endsWith(`/api/projects/${projectId}/runs/${runPayload.run_id}/recover`),
    );
    ...
    if (recoverPostResponse.status() === 200) {
      ...
      expect(recoverPayload.run_id).toBe(runPayload.run_id);
      expect(recoverPayload.status).toBe('completed');
      const recoverDetailResponse = await request.get(`${backendBaseUrl}/api/projects/${projectId}/runs/${recoverPayload.run_id}`);
      expect(recoverDetailResponse.status()).toBe(200);
    } else {
  ...
  test('projects/:project_id/run-monitor route sends cancel request through backend endpoint', async ({
    page,
    request,
  }) => {
    const title = uniqueTitle('e2e-project-run-monitor-cancel-route-contract');
    const project = await createProject(request, title);
    const projectId = project.id;
      ...
      ({ seededProjectId, seededRunId }) => {
        window.localStorage.setItem(
          ...
          JSON.stringify({
            state: {
              projectId: seededProjectId,
              projectTitle: 'Run monitor cancel project',
              selectedMode: 'author',
              chapterCount: 1,
              runId: seededRunId,
            },
    ...
    const cancelPostRequestPromise = page.waitForRequest(
      (networkRequest) =>
        networkRequest.method() === 'POST'
        && networkRequest.url().endsWith(`/api/projects/${projectId}/runs/${runPayload.run_id}/cancel`),
    );
    const cancelPostResponsePromise = page.waitForResponse(
      (networkResponse) =>
        networkResponse.request().method() === 'POST'
        && networkResponse.url().endsWith(`/api/projects/${projectId}/runs/${runPayload.run_id}/cancel`),
    );
    ...
    if (cancelPostResponse.status() === 200) {
      ...
      expect(cancelPayload.run_id).toBe(runPayload.run_id);
  ...
  test('projects/:project_id/run-monitor route loads config preset through backend endpoint', async ({
    ...
    const title = uniqueTitle('e2e-project-run-monitor-config-preset-route-contract');
    const project = await createProject(request, title);
    const projectId = project.id;
      ...
      ({ seededProjectId, seededRunId }) => {
        window.localStorage.setItem(
          ...
          JSON.stringify({
            state: {
              projectId: seededProjectId,
              projectTitle: 'Run monitor preset project',
              selectedMode: 'author',
              chapterCount: 1,
              runId: seededRunId,
            },
    ...
    await page.goto(`/projects/${projectId}/run-monitor`);
    await expect(page.getByRole('button', { name: 'Load run config preset' })).toBeVisible();

    const presetGetRequestPromise = page.waitForRequest(
      (networkRequest) =>
        networkRequest.method() === 'GET'
        && networkRequest.url().endsWith(`/api/projects/${projectId}/runs/${runPayload.run_id}/config-preset`),
    );
    const presetGetResponsePromise = page.waitForResponse(
      (networkResponse) =>
        networkResponse.request().method() === 'GET'
        && networkResponse.url().endsWith(`/api/projects/${projectId}/runs/${runPayload.run_id}/config-preset`),
    );

    await page.getByRole('button', { name: 'Load run config preset' }).click();

    await presetGetRequestPromise;
    const presetGetResponse = await presetGetResponsePromise;
    expect(presetGetResponse.status()).toBe(200);
    const presetPayload = (await presetGetResponse.json()) as { project_id: number; run_id: number };
    expect(presetPayload.project_id).toBe(projectId);
  ...
  test('projects/:project_id/run-monitor route loads pipeline stage durations dashboard through backend endpoint', async ({
    page,
    request,
  }) => {
    const title = uniqueTitle('e2e-project-run-monitor-stage-durations-route-contract');
    const project = await createProject(request, title);
    const projectId = project.id;
      ...
      ({ seededProjectId, seededRunId }) => {
        window.localStorage.setItem(
          ...
          JSON.stringify({
            state: {
              projectId: seededProjectId,
              projectTitle: 'Run monitor stage durations project',
              selectedMode: 'author',
              chapterCount: 1,
              runId: seededRunId,
            },
    ...
    const stageDurationsGetResponsePromise = page.waitForResponse(
      (networkResponse) =>
        networkResponse.request().method() === 'GET' &&
        networkResponse
          .url()
          .endsWith(`/api/projects/${projectId}/runs/${runPayload.run_id}/pipeline-stage-durations-dashboard`),
    );

    await page.goto(`/projects/${projectId}/run-monitor`);
    await expect(page.getByText('Pipeline Stage Durations Dashboard')).toBeVisible();

    const stageDurationsGetResponse = await stageDurationsGetResponsePromise;
    expect(stageDurationsGetResponse.status()).toBe(200);
    const stageDurationsPayload = (await stageDurationsGetResponse.json()) as {
      project_id: number;
      run_id: number;
    };
    expect(stageDurationsPayload.project_id).toBe(projectId);
    expect(stageDurationsPayload.run_id).toBe(runPayload.run_id);
  });

  test('projects/:project_id/run-monitor route loads audiobook prep dashboard through backend endpoint', async ({
    page,
    request,
  }) => {
    const title = uniqueTitle('e2e-project-run-monitor-audiobook-prep-route-contract');
    const project = await createProject(request, title);
    const projectId = project.id;
      ...
      ({ seededProjectId, seededRunId }) => {
        window.localStorage.setItem(
          ...
          JSON.stringify({
            state: {
              projectId: seededProjectId,
              projectTitle: 'Run monitor audiobook prep project',
              selectedMode: 'author',
              chapterCount: 1,
              runId: seededRunId,
            },
    ...
    const audiobookPrepGetResponsePromise = page.waitForResponse(
      (networkResponse) =>
        networkResponse.request().method() === 'GET' &&
        networkResponse.url().endsWith(`/api/projects/${projectId}/runs/${runPayload.run_id}/audiobook-prep-dashboard`),
    );

    await page.goto(`/projects/${projectId}/run-monitor`);
    await expect(page.getByText('Audiobook Prep Dashboard')).toBeVisible();

    const audiobookPrepGetResponse = await audiobookPrepGetResponsePromise;
    expect(audiobookPrepGetResponse.status()).toBe(200);
    const audiobookPrepPayload = (await audiobookPrepGetResponse.json()) as {
      project_id: number;
      run_id: number;
    };
    expect(audiobookPrepPayload.project_id).toBe(projectId);
    expect(audiobookPrepPayload.run_id).toBe(runPayload.run_id);
  });

  test('projects/:project_id/run-monitor route loads character analytics through backend endpoint', async ({
    page,
    request,
  }) => {
    const title = uniqueTitle('e2e-project-run-monitor-character-analytics-route-contract');
    const project = await createProject(request, title);
    const projectId = project.id;
      ...
      ({ seededProjectId, seededRunId }) => {
        window.localStorage.setItem(
          ...
          JSON.stringify({
            state: {
              projectId: seededProjectId,
              projectTitle: 'Run monitor character analytics project',
              selectedMode: 'author',
              chapterCount: 1,
              runId: seededRunId,
            },
    ...
    const characterAnalyticsGetResponsePromise = page.waitForResponse(
      (networkResponse) =>
        networkResponse.request().method() === 'GET' &&
        networkResponse.url().endsWith(`/api/projects/${projectId}/runs/${runPayload.run_id}/character-analytics`),
    );

    await page.goto(`/projects/${projectId}/run-monitor`);
    await expect(page.getByText('Character Analytics Dashboard')).toBeVisible();

    const characterAnalyticsGetResponse = await characterAnalyticsGetResponsePromise;
    expect(characterAnalyticsGetResponse.status()).toBe(200);
    const characterAnalyticsPayload = (await characterAnalyticsGetResponse.json()) as {
      project_id: number;
      run_id: number;
    };
    expect(characterAnalyticsPayload.project_id).toBe(projectId);
    expect(characterAnalyticsPayload.run_id).toBe(runPayload.run_id);
  });

  test('projects/:project_id/run-monitor route loads character cooccurrence graph through backend endpoint', async ({
    page,
    request,
  }) => {
    const title = uniqueTitle('e2e-project-run-monitor-character-cooccurrence-route-contract');
    const project = await createProject(request, title);
    const projectId = project.id;
      ...
      ({ seededProjectId, seededRunId }) => {
        window.localStorage.setItem(
          ...
          JSON.stringify({
            state: {
              projectId: seededProjectId,
              projectTitle: 'Run monitor cooccurrence project',
              selectedMode: 'author',
              chapterCount: 1,
              runId: seededRunId,
            },
    ...
    const cooccurrenceGetResponsePromise = page.waitForResponse(
      (networkResponse) =>
        networkResponse.request().method() === 'GET' &&
        networkResponse
          .url()
          .endsWith(`/api/projects/${projectId}/runs/${runPayload.run_id}/character-cooccurrence-graph`),
    );

    await page.goto(`/projects/${projectId}/run-monitor`);
    await expect(page.getByText('Character Co-occurrence Graph')).toBeVisible();

    const cooccurrenceGetResponse = await cooccurrenceGetResponsePromise;
    expect(cooccurrenceGetResponse.status()).toBe(200);
    const cooccurrencePayload = (await cooccurrenceGetResponse.json()) as {
      project_id: number;
      run_id: number;
    };
    expect(cooccurrencePayload.project_id).toBe(projectId);
