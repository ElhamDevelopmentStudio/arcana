...
from app.services.tagging import detect_emotion_shift, detect_structure

FIXTURES_DIR = Path(__file__).resolve().parent / "fixtures"
RAPID_EMOTION_FIXTURE_PATH = FIXTURES_DIR / "rapid_emotional_shifts.txt"
MIXED_NARRATION_DIALOGUE_FIXTURE_PATH = FIXTURES_DIR / "mixed_narration_dialogue_segment.txt"
...
def _mixed_narration_dialogue_text() -> str:
    text = MIXED_NARRATION_DIALOGUE_FIXTURE_PATH.read_text(encoding="utf-8").strip()
    assert text
    return text
...
def test_detect_structure_identifies_mixed_narration_dialogue_from_fixture() -> None:
    text = _mixed_narration_dialogue_text()
    segment = [line for line in text.splitlines() if line.strip() and not line.startswith("Chapter")][0]

    structure = detect_structure(segment)

    assert structure == "mixed"


def test_pipeline_exports_mixed_narration_dialogue_segments_from_fixture() -> None:
    with TestClient(app) as client:
        project_resp = client.post("/api/projects", json={"title": "Mixed Narration/Dialogue Fixture"})
        assert project_resp.status_code == 201
        project_id = project_resp.json()["id"]

        ingest_resp = client.post(
            f"/api/projects/{project_id}/ingest/txt",
            files={"file": ("mixed-narration-dialogue.txt", io.BytesIO(_mixed_narration_dialogue_text().encode("utf-8")), "text/plain")},
        )
        assert ingest_resp.status_code == 200
        assert ingest_resp.json()["chapter_count"] == 1
        ...
        mixed_segments = [
            segment
            for segment in segments
            if segment.get("type") == "mixed"
        ]
        assert mixed_segments

        mixed_segment = mixed_segments[0]
        assert isinstance(mixed_segment.get("type_confidence"), float)
        assert mixed_segment["type_confidence"] > 0.0
        type_evidence = mixed_segment.get("type_evidence")
        assert isinstance(type_evidence, dict)
        assert type_evidence.get("method") == "rule_based_structure_detection"
