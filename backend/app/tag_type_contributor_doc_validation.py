...
DOC_SECTION_HEADING_PATTERN = re.compile(r"(?m)^##\s+Contributor Guide: How to add a new tag type\s*$")
...
class TagTypeContributorDocValidationError(ValueError):
    pass
...
def extract_contributor_tag_type_section(markdown: str) -> str:
    section_match = DOC_SECTION_HEADING_PATTERN.search(markdown)
    if section_match is None:
        raise TagTypeContributorDocValidationError(
            "Could not find 'Contributor Guide: How to add a new tag type' section."
        )
...
def parse_step_items(section_text: str) -> list[dict[str, object]]:
    ...
    if not headings:
        raise TagTypeContributorDocValidationError("No step headings found in contributor tag-type guide.")
...
def validate_contributor_tag_type_guide(path: Path) -> dict[str, int]:
    markdown = path.read_text(encoding="utf-8")
    section = extract_contributor_tag_type_section(markdown)
    ...
    if len(items) < MIN_STEPS:
        raise TagTypeContributorDocValidationError(
            f"Contributor tag-type guide must include at least {MIN_STEPS} steps, found {len(items)}."
        )
    ...
    if missing_titles:
        raise TagTypeContributorDocValidationError(
            "Contributor tag-type guide is missing required step titles: " + ", ".join(missing_titles)
        )
